...
import { DesignMetadatas } from "./decorator/design";
...
export class ServiceBuilder {
   ...
   * @param implementation 使用指定的实现接口名而不是类本身来注册
   */
   ...
   * @param implementation 使用指定的实现接口名而不是类本身来注册
   */
  constructorInject<T extends {}>(constructor: Constructor<T>, implementation?: string): this {
    const type = implementation || constructor;
   ...
   * @param constructor 类的无参构造函数
   * @param implementation 使用指定的名称而不是类本身来注册
   */
  propertyInject<T extends {}>(constructor: { new(): T }, implementation?: string): this {
    const type = implementation || constructor;
    this._serviceDefs.set(type, new ServiceDescription<T>(
      type,
   ...
   * @param implementation 类型的名称
   */
  instance<T extends object>(instance: T, implementation: string): this {
    this._serviceDefs.set(implementation, new ServiceDescription<T>(
      implementation,
      () => instance,
      false
    ));
